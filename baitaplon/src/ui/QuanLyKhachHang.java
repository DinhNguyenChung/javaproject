...
//		ArrayList<khachhang> listkh = kh_bus.getallkhachhang();
		...
		panel_1.setBounds(10, 10, 1000, 63);
		...
		lblTitle.setBounds(0, 0, 1000, 63);
...
//				cbMaTour.setSelectedItem(table.getValueAt(row, 7));
	...
	private void themActiosn() {
			if(Invalid()) {
				...
				try {
					kh_bus.create(kh);
					String []datarow= {kh.getMaKh(),kh.getTenKh(),kh.getNgsinh().toLocalDate().toString(),kh.getDiachi(),kh.getSdt(),kh.getEmail(),kh.getCmnd()};
